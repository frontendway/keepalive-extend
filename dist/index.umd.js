...
const setTargetMap = (key, value) => {
  targetMap.set(key, value);
};
const getTargetMap = key => targetMap.get(key);
const targetMapHasKey = key => targetMap.has(key);
const deleteTargetMapKey = key => targetMap.delete(key);
...
const getVnodeKey = (vnode, componentOptions) => {
  return vnode.key == null ? componentOptions.Ctor.cid + (componentOptions.tag ? `::${componentOptions.tag}` : '') : vnode.key;
};
...
const isResetCacheKey = (rules, currentRouteName) => {
  ...
  if (refresh && matches(refresh, prevRouteName) || notRefresh && !matches(notRefresh, prevRouteName)) {
    return true;
  }

  return false;
};
// CONCATENATED MODULE: ./lib/keepalive-extend.js

const patternTypes = [String, RegExp, Array];
/* harmony default export */ var keepalive_extend = ({
...
/* harmony default export */ var lib = ({
  install(Vue) {
    Vue.component('keepalive-extend', keepalive_extend);
  }
